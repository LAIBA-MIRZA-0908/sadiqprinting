...
<th class="px-4 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">Order Date</th>
<th class="px-4 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">Designer</th>
 ...
 // Simplified for now
  ...
  for (var i = startIndex; i < endIndex; i++) {
    ...
    html += '<tr class="' + rowClass + '">' +
        ...
        // Order #
        '<td class="px-2 py-1">' +
         '<div class="inline-block px-2 py-1 text-sm font-medium text-white bg-blue-600 rounded-full cursor-pointer hover:bg-blue-700" onclick="viewOrder(' + order.JobOrderNo + ')">JB#' + order.JobOrderNo + '</div>' +
...
'</td>' +
        ...
        '<td class="px-2 py-1">' +
            ...
            (order.status === "pending"
                ? '<button onclick="cancelJob(' + order.JobOrderNo + ')" class="text-red-600 hover:text-red-900 mr-2" title="Cancel">' +
                      '<i class="fas fa-ban"></i>' +
                  '</button>'
                : '<button class="text-gray-400 cursor-not-allowed mr-2" title="Cannot cancel">' +
                      '<i class="fas fa-ban"></i>' +
                  '</button>'
            ) +
...
function printJobOrder(JobOrderNo) {
    ...
    printWindow.onload = function() {
        ...
        // After printing, mark order as printed
        $.post('job_order_functions.php', { action: 'mark_as_printed', JobOrderNo: JobOrderNo }, function(res) {
            if (res.success) {
                loadJobOrders(); // refresh table to show updated status
            } else {
