...
<body class="bg-gray-100">
    <div class="container mx-auto px-4 py-6">
      ...
      <div class="bg-white rounded-lg shadow-md p-4 mb-6 search-filter-box">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select id="dateFilter" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 search-filter-box">
               ...
               <div class="hidden">  <!-- YEH hidden CLASS ADD KAREIN -->
    ...
    <tr>
        <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">Invoice #</th>
            <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider">PO -QN</th>
        ...
        <th class="px-4 py-3 text-right text-sm font-semibold text-gray-700 uppercase tracking-wider">Grand Total</th>
        <th class="px-4 py-3 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider hidden">Status</th>
...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
    ...
    <script>
        ...
        $('#dateFilter').select2({
    dropdownParent: $('.search-filter-box'),
...
<td class="px-2 py-1 cursor-pointer">
    <span class="inline-flex items-center px-2 py-1  text-xs font-semibold">
        ${(invoice.ponumber ?? "") || ""} - ${(invoice.QuotationNo ?? "") || ""}
    </span>
</td>
    ...
    <!-- Made By (Designer) -->
    <td class="px-2 py-1">
  <div class="text-sm text-gray-900">${invoice.madeby ? invoice.madeby : ""}</div>
    </td>

    <!-- Grand Total -->
    <td class="px-2 py-1 text-right">
        <span class="font-semibold">Rs. ${parseFloat(invoice.GrandTotal).toLocaleString('en-PK', { minimumFractionDigits: 2 })}</span>
    </td>

    <!-- Status Badge -->
   <!-- Status Badge -->
<td class="px-2 py-1 hidden">   <!-- YEH hidden CLASS ADD KAREIN -->
    ${statusBadge}
</td>
    ...
    <td class="px-2 py-1 text-center">
        ...
        <button onclick="viewInvoice(${invoice.InvoiceID})"
            class="text-blue-600 hover:text-blue-900 mr-2" title="View">
        ...
        <!-- New Print2 Button -->
        <button onclick="window.open('print2_invoice.php?id=${invoice.InvoiceID}', '_blank')"
            class="text-purple-600 hover:text-purple-800 mr-2" title="Simple Print">
            <i class="fas fa-print"></i> 
        </button>

        <!-- New Print3 Button -->
        <button onclick="window.open('print3_invoice.php?id=${invoice.InvoiceID}', '_blank')"
            class="text-orange-600 hover:text-orange-800 mr-2" title="Previous Balance Print">
            <i class="fas fa-print"></i> 
        </button>

        <button onclick="sendInvoiceWhatsApp('${invoice.Phone}', ${invoice.InvoiceID})"
            class="text-green-500 hover:text-green-700 ml-2" title="Send via WhatsApp">
            <i class="fab fa-whatsapp"></i>
        </button>
<button onclick="window.location.href='edit_invoice.php?id=${invoice.InvoiceID}'"
    class="text-yellow-600 hover:text-yellow-900 mr-2" title="Edit Invoice">
            ...
            var html = '<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">' +
                '<div>' +
                    ...
                    '<div class="bg-gray-50 p-4 rounded-lg">' +
                        ...
                        '<div class="mb-2"><span class="font-medium">Made By:</span> ' + (invoice.madeby || '-') + '</div>' +
                    '</div>' +
                '</div>' +
                '<div>' +
                    ...
                    '<div class="bg-gray-50 p-4 rounded-lg">' +
                        ...
                        '<div class="mb-2"><span class="font-medium">Phone:</span> ' + (maskedPhone || '-') + '</div>' +
            ...
            '<div class="mt-4 bg-gray-50 p-4 rounded-lg">' +
                '<div class="grid grid-cols-2 gap-4">' +
                    ...
                    '<div>' +
                        ...
                        '<div class="mb-2"><span class="font-bold text-lg">Grand Total:</span> Rs. ' + parseFloat(invoice.GrandTotal).toFixed(2) + '</div>' +
                    '</div>' +
                '</div>' +
            '</div>';
...
You can view or download it here: ${invoiceLink}`;
    ...
    <style>
    /* Force search/filter bar to appear above menu dropdown */
    .search-filter-box {
        position: relative;
        z-index: 1;
    }
