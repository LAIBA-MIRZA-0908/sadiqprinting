...
function getJobOrders() {
   ...
   $customerId = isset($_POST['customer_id']) ? (int)$_POST['customer_id'] : 0;
    ...
    $query = "SELECT JobOrderNo, CustomerName, OrderDate, DeliveryDate FROM job_orders WHERE ( status='pending' or  status='printed') ";

    if ($customerId > 0) {
        $query .= "AND CustomerID = " . $customerId;
    }

    $query .= " ORDER BY JobOrderNo DESC";
    $result = $conn->query($query);
    ...
    sendJsonResponse(true, 'Job orders loaded successfully'.$query , [
        'jobOrders' => $jobOrders
    ]);
}
...
$QuotationNo = $_POST['QuotationNo'] ?? null;
...
$CreatedByName = $_POST['CreatedByName'];
    ...
    try {
        ...
        $insertQuery = "INSERT INTO tblinvoices 
            ...
            SubTotal, Advance, GSTRate, TotalGST, NTRRate, TotalNTR, GrandTotal,madeby,PONumber, QuotationNo) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)";
        
        $stmt = $conn->prepare($insertQuery);
        if (!$stmt) throw new Exception('Prepare failed for invoice: ' . $conn->error);
        
        $stmt->bind_param("sssisssddddddsss", $InvoiceNo, $InvoiceDate, $PONo, $CustomerID, $CustomerName,
                          $InvoiceSubject, $SubTotal, $Advance, $GSTRate, $TotalGST, $NTRRate, $TotalNTR, $GrandTotal,$CreatedByName,$PONumber,$QuotationNo);
...
$invoiceData = [
 ...
 // 🧩 Update job order status
        if (!empty($jobNos)) {
            $updateResult = markJobOrdersCompleted($jobNos);
            if (!$updateResult) {
                error_log("⚠️ Failed to update job order status for invoice #{$InvoiceNo}");
            }
        }
...
function markJobOrdersCompleted($jobNos) {
    global $conn;

    // Remove duplicates and empty entries
    $jobNos = array_filter(array_unique($jobNos));
    ...
    $placeholders = implode(',', array_fill(0, count($jobNos), '?'));
    $query = "UPDATE job_orders SET Status = 'completed' WHERE JobOrderNo IN ($placeholders)";
    
    $stmt = $conn->prepare($query);
    if (!$stmt) {
        error_log("Prepare failed in markJobOrdersCompleted: " . $conn->error);
        return false;
    }

    // Bind parameters dynamically
    $types = str_repeat('s', count($jobNos)); // assuming JobNo is VARCHAR; use 'i' if INT
...
function getInvoices() {
    ...
    $query = "SELECT i.InvoiceID, i.InvoiceNo, i.InvoiceDate, i.CustomerName, i.GrandTotal, i.Status, i.PONo,
                     c.Phone, c.Email ,i.madeby,i.ponumber,i.QuotationNo
              FROM tblinvoices i
...
function getInvoiceDetails() {
    ...
    // Get invoice details
    $invoiceQuery = "SELECT i.*, c.Phone, c.Email, c.Address ,i.PONumber,i.QuotationNo 
                     FROM tblinvoices i
                     LEFT JOIN tblcustomers c ON i.CustomerID = c.CustomerID
                     WHERE i.InvoiceID = ?";
...
// ===============================
// ✅ Fetch Job Order List (for modal popup selection)
// ===============================
function get_job_order_list() {
    global $conn;

    $search = trim($_POST['search'] ?? '');
    $query = "SELECT JobOrderNo, CustomerName, OrderDate 
              FROM job_orders";

    if (!empty($search)) {
        $searchTerm = "%{$search}%";
        $query .= " WHERE JobOrderNo LIKE ? OR CustomerName LIKE ?";
        $stmt = $conn->prepare($query);
    ...
    sendJsonResponse(true, 'Job Orders loaded successfully', ['job_orders' => $orders]);
}

// ===============================
// ✅ Fetch Job Order Items (multiple selection support)
// ===============================
function get_job_order_items() {
    global $conn;

    $jobOrderNos = $_POST['job_order_nos'] ?? [];
    if (empty($jobOrderNos)) {
        sendJsonResponse(false, 'No Job Orders selected');
    }

    // Build IN clause dynamically
    $placeholders = implode(',', array_fill(0, count($jobOrderNos), '?'));
    $types = str_repeat('s', count($jobOrderNos));

    $query = "SELECT JobOrderNo AS JobNo, Detail, Media, Width, Height, Qty, Sqft, 0 AS Rate, 0 AS Total
              FROM job_order_details
              WHERE JobOrderNo IN ($placeholders)";
    ...
    $stmt->bind_param($types, ...$jobOrderNos);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    sendJsonResponse(true, 'Job order items loaded successfully', ['items' => $items]);
}

function getNextInvoiceNumber() {
    global $conn;
    
    $query = "SELECT InvoiceNo FROM tblinvoices ORDER BY InvoiceID DESC LIMIT 1";
    $result = $conn->query($query);
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $nextInvoiceNo = 'INV-' . str_pad($nextNumber, 3, '0', STR_PAD_LEFT);
    } else {
        $nextInvoiceNo = 'INV-001';
    }
    
    sendJsonResponse(true, 'Invoice number generated', [
...
function updateInvoice() {
    ...
    if ($InvoiceID <= 0) sendJsonResponse(false, "Invalid invoice ID");
    ...
    $TotalGST = floatval($_POST['TotalGST']);
    $NTRRate = floatval($_POST['NTRRate']);
    $TotalNTR = floatval($_POST['TotalNTR']);
    $GrandTotal = floatval($_POST['GrandTotal']);
    $items = $_POST['items'];

    if (empty($InvoiceDate) || $CustomerID <= 0) {
    ...
    try {
        ...
        $updateQuery = "UPDATE tblinvoices SET 
            InvoiceDate=?, PONo=?, CustomerID=?, CustomerName=?, InvoiceSubject=?, 
        ...
        // DELETE OLD ITEMS
        $conn->query("DELETE FROM tblinvoice_details WHERE InvoiceID = $InvoiceID");

        // INSERT NEW ITEMS
        foreach ($items as $item) {
            $sql = "INSERT INTO tblinvoice_details
                    (InvoiceID, JobNo, Detail, Media, Width, Height, Qty, Sqft, Rate, Total)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

            $stmt2 = $conn->prepare($sql);
            $stmt2->bind_param("isssdddddd",
                $InvoiceID, $item['JobNo'], $item['Detail'], $item['Media'],
                $item['Width'], $item['Height'], $item['Qty'],
                $item['Sqft'], $item['Rate'], $item['Total']
            );
            $stmt2->execute();
        }

        $conn->commit();
        sendJsonResponse(true, "Invoice updated successfully!");

    } catch (Exception $e) {
        $conn->rollback();
        sendJsonResponse(false, "Update failed: " . $e->getMessage());
    }
}
