...
<div class="bg-white rounded-lg shadow-md mb-6">
    <div class="bg-blue-600 text-white px-4 py-2 rounded-t-lg flex items-center">
        <i class="fas fa-file-invoice mr-2 text-sm"></i>
        <h3 class="text-base font-semibold">Job Order Header</h3>
    </div>
</div>
...
<input type="text" id="Designername" name="Designername" 
...
$("#CustomerName").autocomplete({
    minLength: 1, // minimum characters to trigger
    source: function(request, response) {
        // Send AJAX to server to get matching customers
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    // Map results to label/value format
    ...
    select: function(event, ui) {
        ...
        $('#CellNo').val(ui.item.phone); // ya maskPhoneNumber(ui.item.phone)
    }
});
    ...
    $('.media-select').each(function() {
        ...
        if (!$(this).hasClass('select2-hidden-accessible')) {
            $(this).select2({
                placeholder: 'Select or search media',
                allowClear: true,
                width: '100%'
            });
        }
        ...
        function addDetailRow() {
            ...
            var row = '<tr id="detailRow-' + detailRowCount + '">' +
               ...
               '<td class="px-4 py-2"><input type="text" name="Detail[]" class="w-[220px] px-2 py-1 border border-gray-300 rounded" placeholder="Enter detail"></td>'+
                ...
                '<td class="px-4 py-2"><input type="number" name="Width[]" class="w-[60px] px-2 py-1 border border-gray-300 rounded numeric-input" step="1" value="0" onchange="calculateSqft(' + detailRowCount + ')"></td>' +
                '<td class="px-4 py-2"><input type="number" name="Height[]" class="w-[60px] px-2 py-1 border border-gray-300 rounded numeric-input" step="1" value="0" onchange="calculateSqft(' + detailRowCount + ')"></td>' +
                ...
                '<td class="px-4 py-2"><input type="number" name="Sqft[]" class="w-[100px] px-2 py-1 border border-gray-300 rounded numeric-input" step="1" value="0" readonly></td>' +
        ...
        // When user chooses JobFor = Existing Client, show modal
...
function maskPhoneNumber(phone) {
    ...
    return phone.replace(/(\d{3})\d{3,6}(\d{2})/, '$1***$2');
}
    </script>
</body>
</html>